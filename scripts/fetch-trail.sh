...
aws cloudtrail list-public-keys --start-time "$START" --output json > "$OUT/keys.json"
...
for region in regions:
    ...
    if not names:
        continue
    key = prefix + names[-1]
    head = json.loads(subprocess.check_output(["aws", "s3api", "head-object", "--bucket", bucket, "--key", key, "--output", "json"]))
