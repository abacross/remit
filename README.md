...
| Witnessed log (`remit-log`, `remit-logstore`, `remit log`) | Implemented: RFC 9162 tree, C2SP checkpoints, cosignatures, witness and tiles, tested against the RFC, transparency-dev's proof corpus and the reference Go implementations. `remit run` refuses any chain not proven logged, and `remit reconcile` joins only to warrants the log establishes (ADR 0007). Not yet hosted. `remit witness serve` runs a witness over HTTP (c2sp.org/tlog-witness) that anyone can operate, and `remit log append --witness-url` reaches it. Witnesses cosign in Ed25519 or post-quantum ML-DSA-44, checked in both directions against the reference implementation. Abacross's own log is public at https://abacross.github.io/remit-log/ (`scripts/publish-log.sh`) |
