...
3. A witness cosigns with a timestamped Ed25519 key (signature type `0x04`) or, the post-quantum form tlog-cosignature recommends for new deployments, a timestamped ML-DSA-44 key (signature type `0x06`, signing that specification's binary `cosigned_message` over the subtree from 0 to the checkpoint's size), and only after the checks of tlog-witness: the checkpoint is signed by a log key it trusts for the origin; its old size is the size of the last checkpoint it cosigned for that origin; the consistency proof from that checkpoint verifies (section 2.1.4 of RFC 9162); a checkpoint of the same size has the same root; a checkpoint of size zero has the empty tree's root.
