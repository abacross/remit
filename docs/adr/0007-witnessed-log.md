...
- **Signatures.** The log signs with Ed25519 (type `0x01`), which tlog-tiles requires of any log that takes part in the public witness network. Witnesses cosign with timestamped Ed25519 (type `0x04`). tlog-cosignature recommends ML-DSA-44 for new deployments, because Ed25519 is not secure against quantum computers, and Remit's witnesses support it (type `0x06`), added 2026-09-24. Settled then: RustCrypto's `ml-dsa` 0.1.1 states that it "has never been independently audited", so it does not meet ADR 0002's bar; ML-DSA-44 comes from AWS-LC through `aws-lc-rs` 1.18.1, already in the dependency tree through TLS, whose ML-DSA API is stable. It agrees with the reference Go implementation (`filippo.io/mldsa` through `transparency-dev/formats`) in both directions: the same seed gives the same key and key ID, and each verifies the other's cosignatures.
