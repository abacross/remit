...
## Decision, part 3: the agent drafts; the owner approves and sends as himself

Amended 2026-09-24 by the owner, replacing the agent's standing authority to send.
The first version let the marketing agent send on its own authority and sign as itself; the owner chose instead to approve every external message personally and to send it as himself.

**The agent may:** find candidates in public sources; find recipients' addresses only where an organization publishes an email address for contact (a website's contact page, a GitHub organization's public email), never an address scraped from commits or guessed, never a security or vulnerability-report address, and never a contact form (forms are for customers, not for outreach; owner, 2026-09-24); and draft messages, follow-ups and replies for the owner.

**The owner approves every external message**, first contact, follow-up and reply alike, in the approval console: a private page showing each draft in full, with the evidence it was written from, where he approves, edits or rejects it.
A message is sent only if its recipient, subject and body match an approval exactly (`outreach/console.py`, `outreach/send.py`); an edit after approval voids it.
The agent cannot approve, and its writing `status: approved` into a draft counts for nothing.

**Messages go out as Mike Oh, in his voice, and make no claim about how they were written.** Because the owner reads and approves each one before it is sent, each is his message; the earlier objection, an agent writing to strangers under a person's name without saying so, was to the agent sending on its own authority, which no longer happens.
They are sent from a dedicated subdomain address (never the apex, whose reputation carries the company's own mail) with Reply-To an outreach alias that delivers to the owner, so replies reach him directly.

**Limits that stay:** at most ten a day, United States recipients only, each specific to that team and citing the public evidence it was chosen on; one follow-up after seven days without a reply, and nothing after that; no claim about Abacross that is not already published.
Anything involving money, a contract, an NDA, a security questionnaire, access to anyone's systems, a press or public-speaking request, or a complaint is the owner's, as it always was.
...
**Opt-outs.** A reply asking to stop reaches the owner; he records it in the console, and the next sync adds the address to the suppression list, which `send.py` checks before every send. Syncs run at least weekly, inside the ten business days the law allows.

**Everything is recorded:** each approval (who, when, the exact text's hash) in `outreach/approvals.jsonl`, each send in the operations journal (hash-chained and anchored), and each reply's outcome in the program's report.
...
3. An outreach alias in Google Workspace (for example `mike@abacross.com`) that delivers to the owner, used as the Reply-To, and the sending address on the subdomain verified in SES (`OUTREACH_FROM`, for example `Mike Oh <mike@mail.abacross.com>`). No inbound AWS path is needed now that replies go to the owner (amended 2026-09-24).
